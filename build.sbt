...
lazy val core =
  project.settings(
    ...
    libraryDependencies ++= googleDirectoryAPI ++ Seq(
      "software.amazon.awssdk" % "ssm" % "2.26.27" % Test
    )
  )

def projectWithPlayVersion(playVersion: PlayVersion) =
  Project(playVersion.projectId, file(playVersion.projectId)).dependsOn(core).settings(
    Compile / unmanagedSourceDirectories += baseDirectory.value / playVersion.pekkoOrAkkaSrcFolder,

    libraryDependencies ++= Seq(
      "com.gu.play-secret-rotation" %% "core" % "10.1.0",
